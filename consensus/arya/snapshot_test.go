...
// checkpoint creates a arya checkpoint signer section from the provided list
...
// sign calculates a arya digital signature for the given block and embeds it
...
// the account may or may not have cast a arya vote.
...
// Tests that arya signer voting is evaluated correctly for various simple and
// complex scenarios, as well as that a few special corner cases fail correctly.
func TestArya(t *testing.T) {
...
func (tt *aryaTest) run(t *testing.T) {
	...
	config.arya = &params.aryaConfig{
		Period: 1,
		Epoch:  tt.epoch,
	}
	genesis.Config = &config

	engine := New(config.arya, rawdb.NewMemoryDatabase())
	...
	// Pass all the headers through arya and ensure tallying succeeds
